<div class="grid grid-cols-1 lg:grid-cols-2 gap-[135px] items-center mt-[90px]">
    <x-component::input.ckfinder-solution value="{{isset($value) ? $value->image : ''}}"
        name="optimal_body[{{$name}}][image]" id="{{$id}}" />
    <div class="{{isset($order) ? 'order-last lg:order-first' : ''}}">
        <h1 class="uppercase text-black text-[36px] font-bold">
            <input type="text" name="optimal_body[{{$name}}][name]" value="{{isset($value) ? $value->name : ''}}"
        ...
        <ul class="list-disc mr-3 text-[20px] text-[#545454] mt-8">
            ...
            <li><input type="text" name="optimal_body[{{$name}}][list][]" value="{{$list}}" placeholder="Some Text"
                    class="w-full bg-home_light_blue placeholder:text-white"></li>
            @endforeach
            @endif
        </ul>
        ...
        <a class="bg-home_primary inline-block px-[32px] py-[18px] shadow-lg rounded-lg mt-[46px]">
            <div class="flex items-center">
                <span class="text-[24px] font-semibold text-white">
                    <input type="text" name="optimal_body[{{$name}}][button]"
                        value="{{isset($value) ? $value->button : ''}}" placeholder="Some Text"
