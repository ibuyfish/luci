        ...
        <div class="flex-1 p-7">
            ...
            <div class="rounded-lg bg-secondary p-[14px]">
                <div class="mb-5 flex items-center gap-5">
                    <h1 class="text-base">{{ __('dashboard.theme_category.list') }}</h1>
                    ...
                    <div class="w-[120px]">
                        <x-component::button.href href="{{route('admin.theme_category.create')}}"
                    ...
                    <table class="w-[800px] lg:min-w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-text py-3 text-center text-sm uppercase font-medium">
                                    {{ __('dashboard.global.input_thumbnail') }}
                                </th>
                                <th class="text-text py-3 text-center text-sm uppercase font-medium">
                                    {{ __('dashboard.global.input_name') }}
                                </th>
                                <th class="text-text py-3 text-center text-sm uppercase font-medium">
                                    {{ __('dashboard.global.input_category') }}
                                </th>
                                <th class="text-text py-3 text-center text-sm uppercase font-medium">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-text p-3 text-start text-sm">
                                    ...
                                    <img class="w-[80px] h-[80px] mx-auto object-cover"
                                        src="{{env('APP_URL').$model->thumbnail}}" alt="">
                                    @else
                                    <img class="w-[80px] h-[80px] mx-auto object-cover"
                                        src="{{asset('assets/images/empty_img.png')}}" alt="">
                                    @endif
                                </td>
                                <td class="text-text p-3 text-start text-sm">
                                    {{Helper::trans($model,'name')}}
                                </td>
                                <td class="text-text p-3 text-start text-sm">
                                    @if($model->parent_id == null)
                                    {{ __('dashboard.theme_category.parent_title') }}
                                    @else
                                    {{Helper::trans($model->parent,'name')}}
                                    @endif
                                </td>
                                <td class="text-text p-3 text-start text-sm">
                                ...
                                <td tabindex="1"
                                    ...
                                    <div
                                        ...
                                        <div
                                            class="text-text py-[10px] text-start text-sm transition hover:text-primary">
                                            <a href="{{route('admin.theme_category.edit',$model->id)}}">
...
<x-component::modal.delete url="{{route('admin.theme_category.delete')}}" />
...
<script>
    $("#table").on("click", ".setTop", function (e) {
        ...
        $.ajax({
            url: `{{route('admin.theme_category.setTop')}}`,
            type: "POST",
    ...
    $("#table").on("click", ".toggleShow", function (e) {
        ...
        var _token = $('meta[name="csrf-token"]').attr("content");
        $.ajax({
            url: `{{route('admin.theme_category.toggleShow')}}`,
            type: "POST",
            data: { _token: _token, id: id },
            success: function (responese) {
                $("#table").load(location.href + " #table");
